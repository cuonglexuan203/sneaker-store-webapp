    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
        	...
        	Account newAccount = Account.builder()
                    ...
                    .password("newPass")
                    .role(Role.USER)
                    .build(); // Properly populate the new Account object
            long newAccountId = AccountDao.insertOne(newAccount);
            output.append("insertOne: Inserted account with ID: ").append(newAccountId).append("\n");

            // Test insertMany
            Account newAccount2 = Account.builder().username("user2").password("pass2").role(Role.USER).build();
            Account newAccount3 = Account.builder().username("user3").password("pass3").role(Role.USER).build();
            Account newAccount4 = Account.builder().username("user4").password("pass4").role(Role.USER).build();
            Account newAccount5 = Account.builder().username("user5").password("pass5").role(Role.USER).build();
            long insertManyCount = AccountDao.insertMany(Arrays.asList(newAccount2, newAccount3, newAccount4, newAccount5));
            output.append("insertMany: Inserted count: ").append(insertManyCount).append("\n");

            // Test selectOne
            Account account = AccountDao.selectOne(newAccountId); // Use the ID from the insert test
            output.append("selectOne: ").append(account != null ? account.toString() : "No account found").append("\n");

            // Test selectMany
            List<Account> accounts = AccountDao.selectMany();
            output.append("selectMany: ").append(accounts != null ? accounts.toString() : "No accounts found").append("\n");
            ...
            output.append("selectManyByIds: ").append(accountsByIds != null ? accountsByIds.toString() : "No carts found").append("\n");

//             Test updateOne
            if (account != null) {
                account.setRole(Role.ADMIN); // Example update action
                Account updatedAccount = AccountDao.updateOne(account);
                output.append("updateOne: ").append(updatedAccount).append("\n");
            }
            accounts = AccountDao.selectMany();
            output.append("selectMany after updateOne: ").append(accounts != null ? accounts.toString() : "No accounts found").append("\n");
            //
//            Test updateMany
            ...
            accounts = AccountDao.selectMany();
            output.append("selectMany after updateMany: ").append(accounts != null ? accounts.toString() : "No accounts found").append("\n");
            
            
//
//            // Test deleteOne
            long ok = AccountDao.deleteOne(newAccountId); // Use the ID from the insert test
